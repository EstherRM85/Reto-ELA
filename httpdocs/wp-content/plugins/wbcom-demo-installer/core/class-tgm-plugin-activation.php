...
if ( ! class_exists( 'TGM_Plugin_Activation' ) ) {
	...
	class TGM_Plugin_Activation {
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					echo '<p>', sprintf( esc_html( $this->strings['complete'] ), '<a href="' . esc_url( get_wbcom_demo_installer_url() ) . '">' . esc_html__( 'Return to the Theme Installer', 'tgmpa' ) . '</a>' ), '</p>';
					echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
				} else {
					echo '<p><a href="', esc_url( $this->get_wbcom_demo_installer_url() ), '" target="_parent">', esc_html( $this->strings['return'] ), '</a></p>';
				}
		...
		public function get_tgmpa_url() {
			...
			$url = $this->get_wbcom_demo_installer_url();
			return $url;
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		public function no_items() {
			...
			echo esc_html__( 'No plugins to install, update or activate.', 'tgmpa' ) . ' <a href="' . esc_url( $this->get_wbcom_demo_installer_url() ) . '"> ' . esc_html__( 'Return to the Theme Installer', 'tgmpa' ) . '</a>';
			echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
		}
			...
			foreach ( $actions as $action => $text ) {
				$nonce_url = wp_nonce_url(
					add_query_arg(
						...
						$this->tgmpa->get_wbcom_demo_installer_url()
					),
					'tgmpa-' . $action,
					'tgmpa-nonce'
				);
					...
					public function bulk_footer() {
						...
						if ( $this->tgmpa->is_tgmpa_complete() ) {
							...
							$update_actions['dashboard'] = sprintf(
								...
								'<a href="' . esc_url( $this->tgmpa->get_wbcom_demo_installer_url() ) . '">' . esc_html__( 'Return to the Dashboard', 'tgmpa' ) . '</a>'
							);
						} else {
							$update_actions['tgmpa_page'] = '<a href="' . esc_url( $this->tgmpa->get_wbcom_demo_installer_url() ) . '" target="_parent">' . esc_html( $this->tgmpa->strings['return'] ) . '</a>';
						}
