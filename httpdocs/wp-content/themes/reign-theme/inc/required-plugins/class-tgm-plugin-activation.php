	...
	class TGM_Plugin_Activation {
		...
		public function get_download_url( $slug ) {
			...
			switch ( $this->plugins[ $slug ]['source_type'] ) {
				...
				case 'external':
					/* Wbcom Customization :: 16-02-2018 :: start */
					$download_base_url = 'https://demos.wbcomdesigns.com/exporter/plugins/';
					$version_list = get_option( 'reign_external_plugins_versions', array() );
					if( isset( $version_list[$slug] ) ) {
						$download_url = $download_base_url . $slug . '/' . $version_list[$slug] . '/' . $slug . '.zip';
						return $download_url;
					}
					return $this->plugins[ $slug ]['source'];
					/* Wbcom Customization :: 16-02-2018 :: end */
				case 'bundled':
		...
		public function does_plugin_have_update( $slug ) {
			/**
			* Custom code added for external plugins :: start
			*/
			if ( 'external' === $this->plugins[ $slug ]['source_type'] ) {
				$installed_version = $this->get_installed_version( $slug );
				$version_list = get_option( 'reign_external_plugins_versions', array() );
				$version_to_update   = isset( $version_list[$slug] ) ? $version_list[$slug] : '';
				if( version_compare( $version_to_update, $installed_version, '>' ) ) {
					return $version_to_update;
				}
				return false;
			}
			/**
			* Custom code added for external plugins :: end
			*/
